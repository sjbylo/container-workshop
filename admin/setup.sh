...
for i in {0..199}; do useradd user$i \
  -p \
'$6$qmGzVnys$7EGA8ngfhQnHGvNTkeODC6Kw8VvABOYDAfhPua1wSjZJLpDfKi.9g8SAUqLxIyBY5hknSiiXbEZIj5jqgT6Oy.'\
|| break; done

# Log in 100 users
for i in {0..199}; do   su - user$i -c 'oc login --insecure-skip-tls-verify -u user'$i' -p openshift
\
'$MASTER_SVR || break; done

for i in {0..199}; do su - user$i -c 'oc whoami' || break; done

# Delete all
for i in {0..199}; do userdel -r user$i || break; done
